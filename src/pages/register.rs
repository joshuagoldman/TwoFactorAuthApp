...
pub fn Register(cx: Scope, api: UnauthorizedApi) -> impl IntoView {
    let (register_response, set_register_response) =
        create_signal(cx, None::<NewUserResponse>);
    let (register_error, set_register_error) =
        create_signal(cx, None::<String>);
    let (wait_for_response, set_wait_for_response) = create_signal(cx, false);

    let register_action =
        create_action(cx, move |new_user_ref: &NewUser| {
            let new_user = new_user_ref.clone();
            log!("Try to register new account for {:#?}", new_user);
            async move {
...
#[component]
fn BarCode(cx: Scope, new_user_info: NewUserResponse) -> impl IntoView {
    view! {cx,
        <div> 
            <img id="barcode" 
