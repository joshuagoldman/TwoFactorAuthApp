...
use leptos::{
    component, create_action, create_rw_signal, view, ChildrenFn, IntoView, RwSignal, Show,
    SignalGet, SignalUpdate, ViewFn,
};
...
#[component]
pub fn RouteWithApiStateCheck<F>(path: String, view: ApiStateView<F>) -> impl IntoView
where
            ...
            view = move || {
                    ...
                    view! {
                        <Show when = move || api_signals.is_resolved.get()
                                fallback = move || {
                                    view! { <div class="color:red;">{"Loading..."}</div>}
                                }
                        >
                            {view.clone()}
                        </Show>
