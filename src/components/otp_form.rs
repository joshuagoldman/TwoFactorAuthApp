...
{
    let (first_input_signal, first_input_set) = create_signal(cx, None::<String>);
    let (second_input_signal, second_input_set) = create_signal(cx, None::<String>);
    let (third_input_signal, third_input_set) = create_signal(cx, None::<String>);
    let (fourth_input_signal, fourth_input_set) = create_signal(cx, None::<String>);
    let (fifth_input_signal, fifth_input_set) = create_signal(cx, None::<String>);
    let (sixth_input_signal, sixth_input_set) = create_signal(cx, None::<String>);

    let send_otp_enabled = 
        first_input_signal.get().is_some() && second_input_signal.get().is_some() &&
        third_input_signal.get().is_some() && fourth_input_signal.get().is_some() &&
        fifth_input_signal.get().is_some() && sixth_input_signal.get().is_some();
    ...
    let send_otp_ready = Signal::derive(cx, move || {
        first_input_signal.get().is_some() && second_input_signal.get().is_some() &&
        third_input_signal.get().is_some() && fourth_input_signal.get().is_some() &&
        fifth_input_signal.get().is_some() && sixth_input_signal.get().is_some()
    });
...
#[component]
fn OtpInput<F,G>(cx: Scope,
               on_keyup: F,
               on_change: G) -> impl IntoView 
where
